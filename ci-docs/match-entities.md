...
> - Kaip pagrindinį objektą pasirinkite objektą, turintį kelis atributus, būdingus kitiems objektams (pvz., pavadinimą, telefono numerį arba el. pašto adresą).
...
1. Pasirinkite **Objekto tvarka**. Pavyzdžiui, kaip pagrindinį objektą pasirinkite **e-commerce:eCommerceContacts**, o **LoyaltyScheme:loyCustomers** kaip antrąjį objektą. 
1. Jei norite, kad kiekvienas objekto įrašas būtų unikalus klientas ir atitiktų kiekvieną paskesnį objektą, pasirinkite **Įtraukti viską**.
1. Pasirinkite **Atlikta**. 

Nurodžius atitikmenų tvarką, nustatytos rungtynių poros rodomos **"DataUnifyMatch** **·** > **" skyriuje** > **Suderinti įrašai išsami informacija.** Pagrindinė metrika yra tuščia, kol baigsis atitikties procesas.
...
Pirminio objekto *„eCommerce:eCommerceContacts”* sugretinami su kitu objektu *„LoyaltyScheme:loyCustomers”*. Duomenų rinkinys, gautas atlikus pirmąjį atitikties veiksmą, yra suderintas su šiuo objektu, jei turite daugiau nei du objektus.
...
1. Norėdami apibrėžti atitikties taisykles, skyriuje Suderinti įrašai išsami informacija po objektu pasirinkite **Įtraukti** taisyklę **.**
...
1. Srityje **Kurti taisyklę** sukonfigūruokite sąlygas taisyklei.
   ...
   - **Tikslumas**: Nustatykite šiai sąlygai taikytiną tikslumo lygį. 
     - **Pagrindinis**: Pasirinkite iš *Žemas*, *Vidutinis*, *Aukštas* ir *Tikslus*. Pasirinkite **Tiksliai**, kad atitiktumėte tik tuos įrašus, kurie atitinka 100 procentų. Pasirinkite bet kurį kitą lygį, kai norite suderinti įrašus, kurie nėra vienodi 100 procentų.
...
Deduplikuoti įrašai naudojami kryžminio objekto gretinimo procese. Deduplication vyksta atskiruose objektuose ir gali būti konfigūruojamas kiekvienam objektui, naudojamam rungtynių porose.
...
1. Sekcijoje **Išsami įrašų informacija apie** Deduplikuotus įrašus pasirinkite **Nustatyti objektus**. Tuo atveju, jei dublikatų naikinimo taisyklės jau sukurtos, pasirinkite **Redaguoti**.
...
1. Paleidę [atitikties procesą](#run-the-match-process), pagrindinės metrikos plytelėse matysite deduplikacijos statistiką.
...
Jei praturtinote objektus duomenų šaltinis lygiu, pasirinkite juos prieš vykdydami atitikties procesą. Praturtinti subjektai gali pagerinti jūsų suvienijimo rezultatus. Daugiau informacijos ieškokite [Enrichment for data sources](data-sources-enrichment.md). 

Praturtintame objekte yra originalūs duomenų šaltinis laukai ir praturtinti laukai. Taigi, jei nuspręsite dirbti su praturtintu subjektu, esama konfigūracija nebus paveikta. Tačiau gali tekti atnaujinti rungtynių taisykles, kad vietoj to galėtumėte naudoti praturtintus laukus.
...
1. Pasirinkite **Atlikta**. Visur, kur naudojamas šaltinio objektas (pvz., atitikties tvarka arba taisyklės), jis automatiškai pakeičiamas į praturtintą objektą.
...
1. Išplečiamajame sąraše Pasirinktinis **tipas** pasirinkite pasirinktinę atitikties parinktį ir pasirinkite **Atsisiųsti šabloną**. Kiekvienai atitikmens galimybei reikia atskiro šablono.
...
1. Pasirinktinio atitikmens taikymas priklauso nuo norimos naudoti atitikties parinkties. 
   ...
   - Pasirinktinio **aplinkkelio** arba **pseudonimo susiejimui** pasirinkite **Redaguoti** esamą atitikties taisyklę arba sukurkite naują taisyklę. Išplečiamajame sąraše Normalizavimas pasirinkite parinktį Pasirinktinis **aplinkkelis** arba **Pseudonimas ir** pasirinkite **Atlikta**.
...
- Savęs konfiliacija nerodo normalizuotų duomenų deduplicacijos objektuose. Tačiau jis taiko normalizavimą viduje deduplicacijos metu. Tai yra dizainas visiems normalizavimams. 
- Jei semantinio tipo parametras pašalinamas žemėlapio **fazėje**, kai atitikties taisyklėje naudojamas pseudonimo susiejimas arba pasirinktinis aplinkkelis, normalizavimas nebus taikomas. Taip atsitinka tik tuo atveju, jei konfigūruodami atitikties taisyklės normalizavimą išvalysite semantinį tipą, nes semantinis tipas bus nežinomas.


## <a name="next-step"></a>Tolesnis veiksmas

Baigę bent vienos rungtynių poros rungtynių procesą, pereikite prie suliejimo veiksmo [**·**](merge-entities.md).
