---
title: Operacijų mušimas prognozė (yra vaizdo įrašas)
...
> [!TIP]
> Išbandykite operacijų prognozė pamoką naudodami pavyzdinius duomenis: [sandorio prognozė pavyzdžio vadovą](sample-guide-predict-transactional-churn.md).
...
1. Pasirinkite plytelę **Customer churn model** ir pasirinkite **Naudoti šį modelį**.
...
1. Nustatykite **prognozė langą**. Pavyzdžiui, prognozuoti jūsų klientų nutraukimo riziką kitoms 90 dienų siekiant suderinti su jūsų reklamos laikymo pastangomis. Nutraukimo rizikos prognozavimas ilgesniam ar trumpesniam laikotarpiui gali apsunkinti faktorių nustatymą jūsų atsisakymo rizikos profilyje, bet jis priklauso nuo konkrečių verslo reikalavimų.
   >[!TIP]
   > Norėdami įrašyti prognozė kaip juodraštį, bet kuriuo metu galite pasirinkti **Įrašyti juodraštį**. Norėdami tęsti, prognozės juodraštį galite rasti skirtuke **Mano prognozė**.

1. Lauke Churn definition **įveskite dienų skaičių, kad nustatytumėte mušą**. Pavyzdžiui, jei klientas nieko nenupirko per paskutines 30 dienų, jis gali būti laikomas atsisakęs jūsų verslo paslaugų. 
...
1. Dalyje **Pasirinkti veiklas** pasirinkite konkrečias veiklas iš pasirinkto veiklos tipo, į kurį norite sutelkti dėmesį skaičiuojant.
...
1. **Atlikdami veiksmą Įtraukti reikiamus duomenis** pasirinkite **Pirmyn**, kad tęstumėte, jei nenorite įtraukti daugiau veiklų.
